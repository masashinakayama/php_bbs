...
//max関数で$pageより1が大きい時は1を入れる処理(1より下を表示させない記述)
$page = max($page, 1);

//データベースに取得されている件数を調べ、大きな数字を入れられないように最終ページを取得する
$counts = $db->query('SELECT COUNT(*) As cnt FROM posts');
$cnt = $counts->fetch();
//最終のページ数を求める式
$maxPage = ceil($cnt['cnt'] / 5);
//min関数で$maxPageより$pageが大きい時は$maxPageを入れる処理($maxPageより上を表示させない記述)
$page = min($page, $maxPage);

//*5の倍数で?の部分を更新していかなければならない
$start = ($page - 1) * 5;
...
//Reがクリックされた場合,URLパラメータの値を取得しメッセージに表示
...
<?php foreach($posts as $post)://繰り替えしの構文 ?>
    <div class="msg">
    ...
    <p class="day"><a href="view.php?id=<?php print(htmlspecialchars($post['id'], ENT_QUOTES)); ?>">
